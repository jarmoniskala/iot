...
must_haves:
  truths:
    ...
    - "User can edit a room's display name (persisted to Supabase) and icon (persisted to localStorage) from the dashboard"
  ...
  key_links:
    ...
    - from: "components/dashboard/edit-room-dialog.tsx"
      ...
      via: "Updates sensor_config.display_name via Supabase client"
      pattern: "supabase.*sensor_config.*update.*display_name"
    - from: "components/dashboard/edit-room-dialog.tsx"
      to: "localStorage"
      via: "Persists room icon preference keyed by mac_address"
      pattern: "localStorage.*room-icon"
---
...
<task type="auto">
  ...
  <action>
    **EditRoomDialog (`components/dashboard/edit-room-dialog.tsx`):**
    ...
    - On save:
      - **display_name:** Update `sensor_config` table via Supabase client `supabase.from('sensor_config').update({ display_name }).eq('id', sensor.id)`. The RLS UPDATE policy from migration 004 allows this.
      - **icon:** Save icon preference to localStorage via `localStorage.setItem('room-icon-' + sensor.mac_address, iconName)`. Icons are purely a personal UI preference with no database column, so localStorage is the correct storage (keyed by mac_address).
    - On mount, load the current icon from `localStorage.getItem('room-icon-' + sensor.mac_address)`, falling back to the ROOM_ICONS mapping based on room_name from lib/constants.ts.
    - Call `onSave` with the updated sensor config and selected icon so the parent can update local state without waiting for Realtime
    - Show a loading state on the save button during the Supabase call (icon save to localStorage is synchronous)
  ...
  <verify>
    ...
    4. `grep -r "sensor_config.*update" components/dashboard/edit-room-dialog.tsx` confirms Supabase display_name update
    4b. `grep -r "localStorage.*room-icon" components/dashboard/edit-room-dialog.tsx` confirms icon persistence to localStorage
  ...
  <done>
    Room display names are editable from the dashboard via a dialog triggered by the card's edit button, with changes persisted to Supabase sensor_config table. Room icon preferences are editable in the same dialog and persisted to localStorage keyed by mac_address. Weather panel collapses on mobile showing only temperature + condition icon, and expands to show all details. Dark mode toggle is visible in the header and switches between dark and light themes. Sort controls are accessible via a subtle icon in the header.
  </done>
</task>
...
5. Room edit dialog opens on card edit button click, allows changing display name and icon
6. Room display_name edit persists to Supabase sensor_config table; icon edit persists to localStorage keyed by mac_address
...
- Room display name (Supabase) and icon (localStorage) can be edited from the dashboard
