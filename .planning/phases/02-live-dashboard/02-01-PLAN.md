...
must_haves:
  ...
  key_links:
    ...
    - from: "components/dashboard/realtime-provider.tsx"
      ...
      via: "Client component subscribes to Realtime channels for sensor_readings and weather_observations"
      pattern: "postgres_changes.*sensor_readings"
    - from: "components/dashboard/realtime-provider.tsx"
      to: "weather_observations"
      via: "Realtime subscription for weather observation inserts"
      pattern: "postgres_changes.*weather_observations"
...
<task type="auto">
  ...
  <action>
    **Project scaffold:**
    ...
    NOTE: create-next-app auto-generates ~9 scaffold files (package.json, next.config.ts, tsconfig.json, app/layout.tsx, app/page.tsx, app/globals.css, public/*, etc.). These require minimal executor attention beyond the customizations described below. The substantive implementation work in this task focuses on the 13 hand-authored files: lib/supabase/client.ts, lib/supabase/server.ts, lib/types.ts, lib/comfort.ts, lib/weather.ts, lib/staleness.ts, lib/constants.ts, supabase/migrations/004_realtime_and_views.sql, app/providers.tsx, app/error.tsx, .env.local, and the customizations to app/layout.tsx and app/globals.css.
